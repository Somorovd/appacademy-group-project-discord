...
def get_DMs():
    ...
    """
    dms_query = Communication.query.filter(
        or_(
...
def create_new_dm(otherUserId):
    exists = Communication.query.filter(
        or_(
            and_(
                Communication.user1_id == otherUserId,
                Communication.user2_id == current_user.id,
            ),
            and_(
                Communication.user1_id == current_user.id,
                Communication.user2_id == otherUserId,
            ),
        )
    ).one_or_none()
    ...
    if exists is None:
        new_communication = Communication(
            user1_id=current_user.id, user2_id=otherUserId
        )
...
def get_single_dm(communication_id):
    dms_query = DirectMessage.query.filter(
        DirectMessage.communication_id == communication_id
    ).all()
    ...
    if (
        current_user.id != communication["user1Id"]
        and current_user.id != communication["user2Id"]
    ):
        return {"messages": [], "communication": {}}
